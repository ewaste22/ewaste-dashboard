...
export const EditWaste = () => {
  ...
  const getWaste = async () => {
    try {
      ...
      setCategoryId(res.data.data.waste.category_id);
    } catch (err) {}
  };
  ...
  //  }
    ...
    try {
      const res = await axios.put(
        `http://localhost:8000/api/v1/admin/waste/${id}`, form, 
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  {image_waste == null ? (
   <img src={iconupload} className="w-[347px] h-[200px] pt-5" alt="uploadfile" />
    ...
    <img src={image_waste} className="w-[347px] h-[200px] pt-5" alt="uploadfile" />
    )} 
