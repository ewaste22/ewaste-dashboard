...
export default function Courier() {
  ...
  const getCourier = async () => {
    await axios.get('http://localhost:8000/api/v1/admin/courier', {
      headers:{
        Authorization: "Bearer " + localStorage.getItem("token"),
      }
    }).then(res => {
      console.log(res.data.data.courier)
    ...
    <>
      ...
      <div className='container px-8 mx-auto my-10 pb-28'>
        <div className='bg-white border rounded-lg shadow-lg p-10'>
            ...
            <h1 className='text-xl mb-2 font-bold'>Courier</h1>
            </div>
            ...
            <Link to={`/AddCourier`}><button className="text-white absolute right-5  bg-gray-700 hover:bg-gray-800 font-medium rounded-lg text-sm px-4 py-2"> 
        Add</button></Link>
            </div>
            </div>
          ...
          <div className='inline-block min-w-full shadow overflow-hidden'>
            ...
            <tbody>
            {
              ...
              <tr>
                ...
                <td className='px-5 py-5 text-sm text-center'>
                  <p>{item.fullname_courier}</p>
                </td>
                <td className='px-5 py-5 text-sm text-center'>
                  <p>{item.email_courier}</p>
                </td>
                <td className='px-5 py-5 text-sm text-center'>
                  <p>{item.transportationType_courier}</p>
                </td>
